...
def get_vote_average(movie_name):
    ...
    if 'vote_average' in movie_row:
        return movie_row['vote_average'].values[0]
    else:
...
def get_movie_release_date(movie_name):
    ...
    # Check if the column exists
    if 'release_date' in movie_row:
        return movie_row['release_date'].values[0]
    else:
...
# Display Movie Recommendations as Carousel
# Display Movie Recommendations as Fixed-Length Carousel
    ...
    if st.button("Get Recommendations"):
        if user_input:
            ...
            """
             ...
             # Generate each movie item for the carousel
            for _, row in recommendations.iterrows():
                poster_url = get_movie_poster(row['id'])
                movie_title = row['title']
                release_date = get_movie_release_date(movie_title)
                rating = int(get_vote_average(movie_title) * 10)  # Convert to percentage
                ...
                """
                carousel_html += item_html

            carousel_html += "</div>"

            # Display carousel in Streamlit
