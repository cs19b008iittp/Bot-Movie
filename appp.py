...
def getPredictionsV2(user_input, smd, num):
    ...
    return smd.iloc[movie_indices][['title', 'poster_path']]
...
def get_movie_details(movie_name):
    ...
    for i, (_, row) in enumerate(movie_row.iterrows(), start=1):
        ...
        st.image(f"https://image.tmdb.org/t/p/w500{row['poster_path']}", width=200)  # Adjust width as needed
...
if option == "Get Movie Recommendations":
    ...
    if st.button("Get Recommendations"):
        if user_input:
            ...
            # Display the posters and titles in 3 columns
            cols = st.columns(3)
